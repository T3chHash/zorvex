...
function zorvex_install_json(array $payload, int $code = 200): void
{
...
function zorvex_install_locked(): bool
{
    return is_file(zorvex_install_lock_file());
}

function zorvex_install_authorized(): bool
{
    if (!zorvex_install_is_configured()) {
        return true;
    }

    return !empty($_SESSION['zorvex_install_authorized']);
}

function zorvex_install_group(string $group, array $items): array
{
...
$zorvex_install_mutating_actions = ['auth', 'config_write', 'bootstrap', 'probe_begin', 'finish'];
if (in_array($action, $zorvex_install_mutating_actions, true) && ($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    zorvex_install_json(['error' => 'POST required'], 405);
}

if ($action !== '') {
    if (zorvex_install_locked() && $action !== 'state') {
        zorvex_install_json(['error' => 'نصب قبلاً انجام شده است. برای اجرای مجدد فایل install/.installed را حذف کنید.'], 423);
    }

    if ($action === 'state') {
        zorvex_install_json([
            'locked' => zorvex_install_locked(),
            'configured' => zorvex_install_is_configured(),
            'authorized' => zorvex_install_authorized(),
            'shell_exec' => zorvex_install_shell_exec_available(),
            'host' => zorvex_install_host(),
            'base_url' => zorvex_install_base_url(),
    ...
    if ($action === 'auth') {
        ...
        if (!$matches) {
            usleep(700000);
            zorvex_install_json(['ok' => false, 'error' => 'توکن ربات نادرست است.'], 403);
        }

        $_SESSION['zorvex_install_authorized'] = true;
        zorvex_install_json(['ok' => true]);
    }

    if (!zorvex_install_authorized()) {
        zorvex_install_json(['error' => 'برای ادامه ابتدا هویت خود را تأیید کنید.'], 403);
    }

    if ($action === 'requirements') {
        $items = array_merge(
            zorvex_install_group('نسخه PHP', zorvex_install_php_check()),
            zorvex_install_group('وب‌سرور', zorvex_install_webserver_check()),
            zorvex_install_group('اکستنشن‌ها', zorvex_install_extensions_check()),
            zorvex_install_group('تنظیمات PHP', zorvex_install_ini_check())
        );
        zorvex_install_json(zorvex_install_result($items));
    }

    if ($action === 'ssl') {
        zorvex_install_json(zorvex_install_result(zorvex_install_group('دامنه و گواهی', zorvex_install_ssl_check())));
    }

    if ($action === 'paths') {
        zorvex_install_json(zorvex_install_result(zorvex_install_group('ساختار فایل‌ها', zorvex_install_paths_check())));
    }

    if ($action === 'config_load') {
        $values = zorvex_install_config_values();
        foreach ($values as $key => $value) {
            if (zorvex_install_is_placeholder($value)) {
                $values[$key] = '';
            }
        }
        $values['passworddb'] = '';
        ...
        $values['domainhosts'] = zorvex_install_host();
        zorvex_install_json(['ok' => true, 'values' => $values]);
    }
    ...
    if ($action === 'config_check') {
        $values = [
            ...
            'domainhosts' => zorvex_install_host(),
            'usernamebot' => '',
        ];

        if ($values['dbname'] === '' || $values['usernamedb'] === '' || $values['APIKEY'] === '' || $values['adminnumber'] === '') {
            zorvex_install_json(['ok' => false, 'error' => 'نام دیتابیس، کاربر دیتابیس، توکن ربات و آیدی عددی مدیر الزامی هستند.', 'items' => []], 400);
        }
        if ($values['dbhost'] === '') {
            $values['dbhost'] = 'localhost';
        }
        if (!preg_match('/^\d{5,15}$/', $values['adminnumber'])) {
            zorvex_install_json(['ok' => false, 'error' => 'آیدی عددی مدیر باید فقط عدد باشد.', 'items' => []], 400);
        }
        if (!preg_match('/^\d{6,}:[A-Za-z0-9_-]{30,}$/', $values['APIKEY'])) {
            zorvex_install_json(['ok' => false, 'error' => 'قالب توکن ربات نادرست است.', 'items' => []], 400);
        }

        $database = zorvex_install_test_database($values);
        $_SESSION['zorvex_install_values'] = $values;
    ...
    if ($action === 'config_token') {
        $values = $_SESSION['zorvex_install_values'] ?? null;
        if (!is_array($values)) {
            zorvex_install_json(['ok' => false, 'error' => 'اطلاعات فرم یافت نشد؛ صفحه را دوباره باز کنید.', 'items' => []], 400);
        }

        $bot = zorvex_install_telegram($values['APIKEY'], 'getMe');
        if (!$bot['ok']) {
            zorvex_install_json([
                'ok' => false,
                'error' => 'توکن ربات معتبر نیست: ' . $bot['error'],
                'items' => [zorvex_install_item('fail', 'اعتبارسنجی توکن', 'ناموفق', $bot['error'])],
            ], 400);
        }
        ...
        if ($values['usernamebot'] === '') {
            zorvex_install_json([
                ...
                'items' => [zorvex_install_item('fail', 'یوزرنیم ربات', 'دریافت نشد', 'پاسخ تلگرام یوزرنیم نداشت؛ توکن را بررسی کنید.')],
            ], 400);
        }
        $_SESSION['zorvex_install_values'] = $values;

        $items = [
            zorvex_install_item('ok', 'ارتباط با API تلگرام', 'برقرار است'),
            zorvex_install_item('ok', 'ربات شناسایی شد', '@' . $values['usernamebot'], (string) ($bot['result']['first_name'] ?? '')),
        ];

        zorvex_install_json(['ok' => true, 'error' => '', 'items' => $items]);
    }

    if ($action === 'config_write') {
        $values = $_SESSION['zorvex_install_values'] ?? null;
        if (!is_array($values) || $values['usernamebot'] === '') {
            zorvex_install_json(['ok' => false, 'error' => 'ابتدا باید توکن ربات تأیید شود.', 'items' => []], 400);
        }

        $written = zorvex_install_write_config($values);
        if (!$written['ok']) {
            zorvex_install_json([
                'ok' => false,
                'error' => $written['error'],
                'items' => [zorvex_install_item('fail', 'نوشتن config.php', 'ناموفق', $written['error'])],
            ], 500);
        }
        ...
        zorvex_install_json([
            ...
            'error' => '',
            'items' => [
                zorvex_install_item('ok', 'فایل config.php ساخته شد', $values['domainhosts'], 'نسخه قبلی در install/state بکاپ گرفته شد.'),
            ],
        ]);
    }

    if ($action === 'bootstrap') {
        $zorvexRoot = zorvex_install_root();
        $zorvexPreviousDirectory = getcwd();
        @chdir($zorvexRoot);
        @set_time_limit(300);
        $zorvexBootstrapError = '';
        $zorvexBootstrapCompleted = false;

        register_shutdown_function(static function () use (&$zorvexBootstrapCompleted) {
            if ($zorvexBootstrapCompleted) {
                return;
            }
            ...
            echo json_encode([
                ...
                'items' => [zorvex_install_item('fail', 'ساخت جداول', 'متوقف شد', $output !== '' ? $output : 'پاسخی از سرور دریافت نشد.')],
            ], JSON_UNESCAPED_UNICODE);
        });

        ob_start();
        try {
            require_once $zorvexRoot . '/db/bootstrap.php';
        } catch (Throwable $zorvexBootstrapException) {
            $zorvexBootstrapError = $zorvexBootstrapException->getMessage();
        }
        ob_end_clean();
        $zorvexBootstrapCompleted = true;

        if ($zorvexPreviousDirectory !== false) {
            @chdir($zorvexPreviousDirectory);
        }

        if ($zorvexBootstrapError !== '') {
            zorvex_install_json([
                'ok' => false,
                'error' => 'ساخت جداول ناموفق بود: ' . $zorvexBootstrapError,
                'items' => [zorvex_install_item('fail', 'ساخت جداول', 'ناموفق', $zorvexBootstrapError)],
            ], 500);
        }
        ...
        zorvex_install_json([
            ...
            'error' => '',
            'items' => [
                zorvex_install_item('ok', 'جداول دیتابیس', 'ساخته و به‌روزرسانی شد', 'جداول، ایندکس‌ها و مهاجرت‌ها اعمال شدند.'),
                zorvex_install_item('ok', 'وبهوک تلگرام', 'در مرحله پایانی ست می‌شود'),
            ],
        ]);
    }
    ...
    if ($action === 'cron_plan') {
        zorvex_install_json([
            'ok' => true,
            'jobs' => zorvex_install_cron_plan(),
            'required' => zorvex_install_required_jobs(),
            'probe' => zorvex_install_probe_status(),
        ]);
    }

    if ($action === 'probe_begin') {
        zorvex_install_probe_reset();
        zorvex_install_json(zorvex_install_probe_status());
    }
    ...
    if ($action === 'finish') {
        if (!zorvex_install_shell_exec_available()) {
            $probe = zorvex_install_probe_status();
            if (!$probe['verified']) {
                zorvex_install_json(['ok' => false, 'error' => 'اجرای کرون هاست هنوز تأیید نشده است.'], 400);
            }

            $confirmed = json_decode((string) ($_POST['confirmed'] ?? '[]'), true);
            $confirmed = is_array($confirmed) ? array_map('strval', $confirmed) : [];
            $missing = array_diff(zorvex_install_required_jobs(), $confirmed);
            if ($missing !== []) {
                zorvex_install_json(['ok' => false, 'error' => 'این کرون‌ها هنوز تأیید نشده‌اند: ' . implode('، ', $missing)], 400);
            }
        }

        if (!zorvex_install_is_configured()) {
            zorvex_install_json(['ok' => false, 'error' => 'ابتدا باید مرحله تنظیمات ربات کامل شود.'], 400);
        }

        $values = zorvex_install_config_values();
        $webhookUrl = 'https://' . $values['domainhosts'] . '/index.php';
        $webhookSecret = zorvex_install_webhook_secret($values);
        $webhookTarget = $webhookSecret === ''
            ? $webhookUrl
            : $webhookUrl . '?secret=' . rawurlencode($webhookSecret);
        $reactivateUrl = 'https://' . $values['domainhosts'] . '/table.php';

        @file_put_contents(zorvex_install_lock_file(), (string) time());
        $deleted = zorvex_install_delete_tree(__DIR__);

        if (!$deleted) {
            zorvex_install_telegram($values['APIKEY'], 'deleteWebhook', []);
        ...
        $webhook = zorvex_install_telegram($values['APIKEY'], 'setWebhook', [
            'url' => $webhookTarget,
            'max_connections' => 40,
        ]);
        if (!$webhook['ok']) {
        ...
        $info = zorvex_install_telegram($values['APIKEY'], 'getWebhookInfo');
        if ($info['ok']) {
        ...
        zorvex_install_telegram($values['APIKEY'], 'sendMessage', [
            'chat_id' => $values['adminnumber'],
            'text' => 'ربات هوشمند Zorvex روی هاست نصب شد. برای شروع دستور /start را بفرستید.',
        ]);
    ...
    zorvex_install_json(['error' => 'درخواست نامعتبر است.'], 400);
}
...
<head>
    ...
    <title>نصب ربات هوشمند Zorvex</title>
    <style>
        @font-face {
...
<body>
    ...
    <?php if (!$locked && $authorized): ?>
        <script>
            const SHELL_EXEC_AVAILABLE = <?php echo zorvex_install_shell_exec_available() ? 'true' : 'false'; ?>;
            const STEPS = [

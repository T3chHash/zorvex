...
$brandAppVersion = trim((string)@file_get_contents(__DIR__ . '/version')) ?: '1.0.6';


const FX_DEFAULT_BRAND_NAME = 'zorvex';
$brandName = FX_DEFAULT_BRAND_NAME;
$brandMark = 'Z';
$brandTitle = 'Zorvex Pro';
$brandLogoUrl = '';
...
<head>
    ...
    (function () {
        var THEMES = {
            cyber:  { c: '#00f2fe', b: '#4facfe' },
            purple: { c: '#7c5cff', b: '#a98bff' },
            gold:   { c: '#d4b878', b: '#e2c98c' },
            emerald:{ c: '#00e5a3', b: '#3bf7be' },
            blue:   { c: '#64a8e8', b: '#82bdf3' },
            red:    { c: '#ff477e', b: '#ff759f' },
        ...
        try {
            ...
            else { key = 'cyber'; t = THEMES[key] || THEMES.cyber; }
            function onAcc(h){var m=/^#([0-9a-f]{6})$/i.exec(h);if(!m)return '#ffffff';var n=parseInt(m[1],16);function lin(v){v/=255;return v<=0.03928?v/12.92:Math.pow((v+0.055)/1.055,2.4);}var L=0.2126*lin((n>>16)&255)+0.7152*lin((n>>8)&255)+0.0722*lin(n&255);return L>0.45?'#0a0b10':'#ffffff';}
            ...
            window.__ZORVEX_MODE__ = mode;
            var mc = document.querySelector('meta[name="theme-color"]');
            if (mc) mc.setAttribute('content', '#0f1117');
        } catch (e) {
        }
    })();
</script>
...
(function () {
    window.__hardReload = function () {
        ...
        try {
            var preserveKeys = ['zorvex.theme.accent', 'zorvex.theme.mode', 'zorvex.initData', 'zorvex.initDataUnsafe', 'faoxima.theme.accent', 'faoxima.theme.mode'];
            var keep = {};
        ...
        <main id="view" class="view" aria-live="polite">
            <article class="card card-window">
                <header class="card-window-bar">
                    ...
                    <span class="window-url">zorvex/pro</span>
                </header>
                <div class="card-body">

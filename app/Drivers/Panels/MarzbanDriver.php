...
class MarzbanDriver implements PanelInterface
{
    ...
    {
        $ch = curl_init("{$this->baseUrl}/api/admin/token");
    ...
    public function createUser(string $username, int $trafficBytes, int $expireTimestamp, array $inbounds = []): array
    {
        ...
        $res = $this->request('api/user', 'POST', $payload);
        ...
        return [
            'success' => false,
            'sub_url' => '',
            'error' => $res['data']['detail'] ?? 'Could not create user on Marzban',
        ];
    }

    public function getUser(string $username): ?array
    {
        $res = $this->request("api/user/{$username}");
    ...
    {
        $res = $this->request("api/user/{$username}/reset", 'POST');
        return $res['ok'];
    }

    public function updateUser(string $username, array $data): bool
    {
        $res = $this->request("api/user/{$username}", 'PUT', $data);
        return $res['ok'];
    }

    public function deleteUser(string $username): bool
    {
        $res = $this->request("api/user/{$username}", 'DELETE');
        return $res['ok'];
    }

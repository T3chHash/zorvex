...
if (in_array($text, $textadmin) || $datain == "admin") {
    ...
    if ($userdata['type'] == "hiddify") {
        ...
        savedata("save", "password", "null");
        return;
    } elseif ($userdata['type'] == "s_ui" || $userdata['type'] == "WGDashboard" || $userdata['type'] == "x-ui_single" || $userdata['type'] == "zorvex_agent" || $userdata['type'] == "rebecca") {
...
elseif ($datain == "systemsms") {
    ...
    } elseif ($marzban_list_get['type'] == "zorvex_agent") {
        sendmessage($from_id, $textbotlang['users']['selectoption'], $option_zorvex, 'HTML');
...
if ($datain == "settimecornday" && $adminrulecheck['rule'] == "administrator") {
    ...
    foreach ($list_panel as $result) {
        $list_panel_keyboard['inline_keyboard'][] = [
            ['text' => $result['name_panel'], 'callback_data' => "set_panel_id_zorvex:{$result['id']}:{$panel['id']}"],
        ];
    }
    sendmessage($from_id, $textbotlang['Admin']['addorder']['selectPanel'], json_encode($list_panel_keyboard), 'HTML');
} elseif (preg_match('/set_panel_id_zorvex:(.*):(.*)/', $datain, $dataget)) {
    $panel_id_zorvex = $dataget[1];
    $panel_id = $dataget[2];
    deletemessage($from_id, $message_id);
    update("marzban_panel", "inbounds", $panel_id_zorvex, "id", $panel_id);
    sendmessage($from_id, $textbotlang['Admin']['addorder']['panelSelected'], $option_zorvex, 'HTML');

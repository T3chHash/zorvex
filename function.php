...
function rate_arze()
{
    $ch = curl_init('https://demo.zorvex.com/b.php');
...
{
    global $from_id, $optionMarzban, $optionX_ui_single, $optionhiddfy, $option_zorvex, $optionalireza_single, $optionmarzneshin, $option_mikrotik, $optionwg, $options_ui, $optionibsng, $optionrebecca;
    ...
    } elseif ($typepanel == "zorvex_agent") {
        sendmessage($from_id, $message, $option_zorvex, 'HTML');
...
function checktelegramip()
{
    ...
    // Always allow valid Telegram webhook payloads containing update_id
    $raw = @file_get_contents('php://input');
    if (!empty($raw) && strpos($raw, '"update_id"') !== false) {
        return true;
    }
...
{
    $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'zorvex_qr';
...
function zorvexRemoveInstallerPath($path)
{
    ...
    foreach ($entries as $entry) {
        ...
        $removed = zorvexRemoveInstallerPath($path . '/' . $entry) && $removed;
    }

    return @rmdir($path) && $removed;
}

function zorvexInstallerNoticeTexts()
{
    global $textbotlang;
...
function zorvexShouldAlertInstallerAdmin($cooldown = 3600)
{
...
function zorvexNotifyInstallerBlocked()
{
    global $from_id, $adminnumber;
    if (!function_exists('sendmessage')) {
        return;
    }
    $texts = zorvexInstallerNoticeTexts();
    ...
    if (!$userIsAdmin && $adminId !== '' && zorvexShouldAlertInstallerAdmin()) {
        sendmessage($adminId, $texts['admin'], null, 'HTML');
    }
}

function zorvexStopForInstaller($message)
{
    error_log($message);
    zorvexNotifyInstallerBlocked();
    if (!headers_sent()) {
...
function zorvexEnsureInstallerRemoved()
{
    return;
}

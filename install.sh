...
if [[ $EUID -ne 0 ]]; then
    echo -e "\033[31m[خطا]\033[0m لطفاً این اسکریپت را با دسترسی \033[1mroot\033[0m اجرا نمایید."
    exit 1
fi
...
_kv()     { printf "    ${C_DIM}%-18s${CR}${C_BORDER}:${CR} %b${CR}\n" "$1" "$2"; }
_dot() {
    case "$1" in
        ok)   printf "${C_OK}●${CR} "  ;;
        bad)  printf "${C_BAD}●${CR} " ;;
...
run_step() {
    ...
    if [ "$exit_code" -eq 0 ]; then
        ...
        tail -n 15 "$INSTALL_LOG" 2>/dev/null
...
get_domain() {
    if [ -f "$APP_DIR/config/app.php" ]; then
        grep "'url'" "$APP_DIR/config/app.php" | head -1 | cut -d"'" -f4 | sed -e 's|^https://||' -e 's|^http://||'
    else
        echo ""
    fi
}
...
status_dashboard() {
    ...
    if is_installed; then
        ...
        _kv "دامنه سرور" "${C_DIM}https://${dom}${CR}"
        ...
        if [ -n "$dom" ] && [ -f "/etc/letsencrypt/live/$dom/cert.pem" ]; then
            ...
            if [ "$days" -gt 10 ]; then
                _kv "گواهی SSL" "$(_dot ok)${C_OK}دارای اعتبار (${days} روز باقی‌مانده)${CR}"
            else
                _kv "گواهی SSL" "$(_dot warn)${C_WARN}نزدیک به انقضا (${days} روز)${CR}"
            fi
        else
            _kv "گواهی SSL" "$(_dot warn)${C_WARN}ثبت نشده یا نامعتبر${CR}"
        fi
        ...
        if [ -n "$token" ]; then
            ...
            if [ -n "$url" ]; then
                _kv "وب‌هوک تلگرام" "$(_dot ok)${C_OK}متصل${CR}"
            else
                _kv "وب‌هوک تلگرام" "$(_dot bad)${C_BAD}ست نشده${CR}"
            fi
        fi
    else
...
install_bot() {
    banner
    _sec "نصب و راه‌اندازی Zorvex"

    if is_installed; then
        echo -e "  ${C_WARN}!${CR} ${C_WARN}سیستم Zorvex در حال حاضر روی این سرور نصب است.${CR}"
        printf "  ${C_PROMPT}❯${CR} آیا مایل به نصب مجدد و بازنویسی اطلاعات هستید؟ ${C_DIM}[y/N]${CR}: "
        read -r _confirm
        if [[ ! "$_confirm" =~ ^[Yy]$ ]]; then
            show_menu
            return 0
    ...
    echo -e "  ${C_DIM}لطفاً اطلاعات زیر را با دقت وارد فرمایید:${CR}\n"
    ...
    echo ""
    run_step "بروزرسانی مخازن لینوکس" "apt-get update -y"
    run_step "نصب بسته‌های پایه (git, curl, nginx, mysql)" "apt-get install -y software-properties-common curl git unzip nginx certbot python3-certbot-nginx mysql-server"
    run_step "افزودن مخزن PHP و نصب PHP 8.3" "add-apt-repository -y ppa:ondrej/php && apt-get update -y && apt-get install -y php8.3-fpm php8.3-cli php8.3-mysql php8.3-curl php8.3-mbstring php8.3-xml php8.3-zip"

    run_step "ایجاد دیتابیس و دسترسی‌های کاربر" \
        "mysql -e \"CREATE DATABASE IF NOT EXISTS \`${DB_NAME}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci; CREATE USER IF NOT EXISTS '${DB_USER}'@'localhost' IDENTIFIED BY '${DB_PASS}'; GRANT ALL PRIVILEGES ON \`${DB_NAME}\`.* TO '${DB_USER}'@'localhost'; FLUSH PRIVILEGES;\""

    # Clone or copy files
    if [ ! -f "database/schema.sql" ]; then
        run_step "دریافت آخرین کدهای پروژه از گیت‌هاب" "rm -rf '${APP_DIR}' && git clone https://github.com/${GIT_REPO}.git '${APP_DIR}'"
    else
        mkdir -p "${APP_DIR}"
        cp -r ./* "${APP_DIR}/" || true
    fi

    run_step "واردسازی جداول پیش‌فرض دیتابیس" "mysql -u'${DB_USER}' -p'${DB_PASS}' '${DB_NAME}' < '${APP_DIR}/database/schema.sql'"
...
EOF
    ...
    # SSL Issuance
    if [ -n "$SSL_EMAIL" ]; then
        run_step "دریافت گواهی امنیتی SSL (Let's Encrypt)" "certbot --nginx -d '${DOMAIN_NAME}' --non-interactive --agree-tos -m '${SSL_EMAIL}' --redirect || true"
    fi

    # Set Webhook
    run_step "اتصال وب‌هوک تلگرام" \
    ...
    run_step "تنظیم سرویس زمان‌بندی (Cron)" "systemctl enable cron 2>/dev/null; systemctl start cron 2>/dev/null || true"
    ...
    echo ""
    _sec "اطلاعات نصب و دسترسی"
    _kv "آدرس وب‌سایت" "${C_KEY}https://${DOMAIN_NAME}${CR}"
    _kv "کنترل‌پنل وب ادمین" "${C_KEY}https://${DOMAIN_NAME}/admin/index.php${CR}"
    _kv "مینی‌اپ تلگرام" "${C_KEY}https://${DOMAIN_NAME}/miniapp/index.php${CR}"
    _kv "دستور مدیریت در سرور" "${C_OK}zorvex${CR}"
    echo ""
...
update_bot() {
    ...
    run_step "بروزرسانی ساختار دیتابیس" "cd '$APP_DIR' && php -r 'require_once \"app/autoload.php\"; Zorvex\\Database\\MigrationRunner::run();'"
...
backup_db() {
    ...
    if [ -n "$token" ] && [ -n "$admin_id" ]; then
        run_step "ارسال فایل پشتیبان به چت تلگرام ادمین" \
...
change_settings() {
    banner
    _sec "تغییر اطلاعات و تنظیمات ربات"
    if ! is_installed; then
        echo -e "  ${C_BAD}●${CR} ${C_BAD}ربات نصب نیست.${CR}"
        sleep 2; show_menu; return 1
    fi

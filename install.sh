...
banner()  {
    echo
    _drule
    printf "  ${C_OK}▌${CR} ${C_TITLE}ZORVEX PRO${CR}  ${C_DIM}— VPN Subscription Management Platform${CR}\n"
    _drule
}
...
ensure_dns() {
    ...
    elif [ -f "$RESOLV" ] && [ ! -f "${RESOLV}.zorvex.bak" ]; then
        cp -a "$RESOLV" "${RESOLV}.zorvex.bak" 2>/dev/null
    fi
...
# Ensure /usr/local/bin/zorvex points at the master script
_link_zorvex() {
...
function self_update_script() {
    ...
    local BIN_LINK="/usr/local/bin/zorvex"
    local URL="https://raw.githubusercontent.com/T3chHash/zorvex/main/install.sh"
    local TEMP_FILE="/tmp/zorvex_update.sh"
    ...
    if [ "$valid" -ne 1 ]; then
        ...
        _link_zorvex "$MASTER_PATH" "$BIN_LINK"
        return 0
    fi
...
# ── Repo / paths ─────────────────────────────────────────────
BOT_DIR_DEFAULT="/var/www/html/zorvexbotconfig"
CONFIG_FILE_DEFAULT="$BOT_DIR_DEFAULT/config.php"
GIT_REPO="T3chHash/zorvex"
LATEST_CACHE="/tmp/.zorvex_latest_version"
IP_CACHE="/tmp/.zorvex_server_ip"
...
STATE_DIR="/root/confzorvex"
STATE_FILE="$STATE_DIR/.zorvex_install_state"
...
php_repo_disable() {
    ...
    for f in /etc/apt/sources.list.d/*ondrej*php*.sources /etc/apt/sources.list.d/*ondrej*php*.list; do
        [ -f "$f" ] || continue
        mv -f "$f" "$f.disabled-by-zorvex" && n=$((n + 1))
    done
    [ "$n" -gt 0 ]
}
...
setup_mysql_root() {
    sudo mkdir -p /root/confzorvex || return 1
    sudo chmod 700 /root/confzorvex || return 1
    touch /root/confzorvex/dbrootzorvex.txt || return 1
    sudo chmod 600 /root/confzorvex/dbrootzorvex.txt || return 1
    ...
    echo "\$user = 'root';"               >> /root/confzorvex/dbrootzorvex.txt
    echo "\$pass = '${randomdbpasstxt}';" >> /root/confzorvex/dbrootzorvex.txt
    echo "\$path = '${RANDOM_NUMBER}';"   >> /root/confzorvex/dbrootzorvex.txt
    passs=$(grep '$pass' /root/confzorvex/dbrootzorvex.txt | cut -d"'" -f2)
    userrr=$(grep '$user' /root/confzorvex/dbrootzorvex.txt | cut -d"'" -f2)
    local alter_ok=0
    ...
    local dropin="$dropin_dir/zz-zorvex-recovery.cnf"
...
# Only runs on a brand-new install (never on resume / Zorvex's own partial state).
...
install_pause() {
    ...
    echo -e "      ${C_KEY}zorvex install${CR}"
...
function renew_ssl() {
    ...
    local cfg="/var/www/html/zorvexbotconfig/config.php"
    local domain=""
    if [ -f "$cfg" ]; then
    ...
    if ! command -v certbot >/dev/null 2>&1; then
        echo -e "  ${C_BAD}●${CR} ${C_BAD}certbot is not installed. Install Zorvex first.${CR}"
        sleep 1; show_menu; return 1
    fi
...
function backup_bot() {
    ...
    CONFIG_PATH="/var/www/html/zorvexbotconfig/config.php"
    if [ ! -f "$CONFIG_PATH" ]; then
        printf "    ${C_BAD}●${CR} ${C_BAD}Zorvex is not installed. config.php not found.${CR}\n"
        echo ""
        printf "  ${C_PROMPT}❯${CR} Press Enter to return to the menu... "
        read -r _
        show_menu
    ...
    local backup_file="/root/zorvex_backup_${backup_date}.sql"
    ...
    if [ -n "$bot_token" ] && [ -n "$admin_id" ]; then
        ...
        send_result=$(curl -s -o /dev/null -w "%{http_code}" \
            ...
            -F "caption=📦 Zorvex DB Backup (${backup_date})" \
...
function import_bot() {
    ...
    while IFS= read -r f; do
        ...
        i=$((i + 1))
    done < <(find /root -maxdepth 1 -name 'zorvex_backup_*.sql' -type f 2>/dev/null | sort -r)
...
function show_menu() {
    ...
    if [ -n "$inst" ] && [ -n "$latest" ] && [ "$inst" != "$latest" ]; then
        echo ""
        echo -e "  \033[1;43;30m 🔔 NEW UPDATE AVAILABLE: \033[0m \033[1;33mVersion $latest released! (Installed: $inst)\033[0m"
        echo -e "  \033[1;36m👉 Select option [2] to automatically update to version $latest.\033[0m"
        echo ""
        update_badge="  \033[1;33m[● Update to $latest available]\033[0m"
    fi
    _sec "Menu"
...
function show_help_screen() {
    ...
    _sec "Commands"
    _kv "install" "${C_DIM}Install Zorvex${CR}"
    _kv "update" "${C_DIM}Update Zorvex (choose channel / version)${CR}"
    _kv "pass" "${C_DIM}View or reset admin panel password${CR}"
    _kv "remove" "${C_DIM}Remove Zorvex and its services${CR}"
    ...
    _kv "--version" "${C_DIM}Specific release tag (e.g. 1.0.0)${CR}"
    _kv "--channel" "${C_DIM}beta | release | auto${CR}"
    _kv "-h, --help" "${C_DIM}Show CLI help and exit${CR}"

    _sec "Examples"
    printf "    ${C_KEY}zorvex update${CR}\n"
    printf "    ${C_KEY}zorvex pass${CR}\n"
    printf "    ${C_KEY}zorvex install --channel auto${CR}\n"
    printf "    ${C_KEY}zorvex install --token 123:ABC \\\\${CR}\n"
    printf "    ${C_DIM}            --admin 111 --domain bot.example.com --version 1.0.0${CR}\n"
    printf "    ${C_KEY}zorvex update --version 1.0.0${CR}\n"
    printf "    ${C_KEY}zorvex remove${CR}\n"
    printf "    ${C_KEY}zorvex backup${CR}\n"
    printf "    ${C_KEY}zorvex import${CR}\n"
...
function fix_update_issues() {
    ...
    [ -f "$target" ] && cp "$target" "$target.zorvexbackup"

    local parked=""
    if [ "$fmt" = "deb822" ] && [ -s "$LEGACY" ]; then
        cp "$LEGACY" "$LEGACY.zorvexbackup" && : > "$LEGACY" && parked="$LEGACY"
    fi
...
EOF
        ...
        if apt-get update --allow-releaseinfo-change 2>/dev/null; then
            ...
            rm -f "$target.zorvexbackup"
            [ -n "$parked" ] && rm -f "$parked.zorvexbackup"
            return 0
        fi
    done
    if [ -f "$target.zorvexbackup" ]; then
        mv "$target.zorvexbackup" "$target"
    else
        rm -f "$target"
    fi
    [ -n "$parked" ] && [ -f "$parked.zorvexbackup" ] && mv "$parked.zorvexbackup" "$parked"
...
VPNBOT_BACKUP="/tmp/zorvex_vpnbot_backup"
...
function install_bot() {
    ...
    if [ -f "$CONFIG_FILE_DEFAULT" ] && ! has_resumable_state; then
        ...
        printf "    ${C_BAD}●${CR} ${C_BAD}Zorvex is already installed on this server.${CR}\n"
    ...
    if ! phase_done FILES; then
        ...
        CURRENT_RUN_DIR="$(pwd)"
        if [ -f "$CURRENT_RUN_DIR/index.php" ] && [ -f "$CURRENT_RUN_DIR/panels.php" ] && [ "$CURRENT_RUN_DIR" != "$BOT_DIR" ]; then
            echo -e "  ${C_OK}●${CR} ${C_DIM}Found local Zorvex source files in $CURRENT_RUN_DIR. Copying...${CR}"
            cp -r "$CURRENT_RUN_DIR"/* "$BOT_DIR/" 2>/dev/null || true
            cp -r "$CURRENT_RUN_DIR"/.[!.]* "$BOT_DIR/" 2>/dev/null || true
            purge_installer_dir "$BOT_DIR"
        else
            TEMP_DIR="/tmp/zorvexbot"
            rm -rf "$TEMP_DIR"; mkdir -p "$TEMP_DIR"
            run_step "Downloading Zorvex (${SRC_LABEL_RESUME})" "wget -O '$TEMP_DIR/bot.zip' '$ZIP_URL'" \
    ...
    if ! phase_done DBROOT; then
        if [ ! -f "/root/confzorvex/dbrootzorvex.txt" ] || ! grep -q '\$pass' /root/confzorvex/dbrootzorvex.txt 2>/dev/null; then
    ...
    PATHS=$(cat /root/confzorvex/dbrootzorvex.txt | grep '$path' | cut -d"'" -f2)
...
EOF
    ...
    ROOT_PASSWORD=$(cat /root/confzorvex/dbrootzorvex.txt | grep '$pass' | cut -d"'" -f2)
    ROOT_USER="root"
    ...
    if ! phase_done CONFIG; then
        wait
        sleep 1
        file_path="/var/www/html/zorvexbotconfig/config.php"
        if [ -f "$file_path" ]; then
            rm "$file_path" || {
        ...
        sleep 1
        cat <<EOF > /var/www/html/zorvexbotconfig/config.php
<?php
...
?>
EOF
        sudo chown www-data:www-data /var/www/html/zorvexbotconfig/config.php 2>/dev/null
        sudo chmod 640 /var/www/html/zorvexbotconfig/config.php 2>/dev/null
        mark_phase CONFIG
    else
    ...
    if ! phase_done WEBHOOK; then
        ...
        MESSAGE="✅ The Zorvex bot is installed! for start the bot send /start command."
    ...
    printf "    ${C_OK}●${CR} ${C_OK}Zorvex is installed and the webhook is set.${CR}\n"
    ...
    _sec "Manage"
    _kv "Command" "${C_DIM}run ${CR}${C_KEY}zorvex${CR}${C_DIM} anytime to open this panel${CR}"
    echo ""
    _rule
    echo ""

    chmod +x /root/install.sh
    ln -sf /root/install.sh /usr/local/bin/zorvex
    self_update_script
}
function update_bot() {
    clear
    ...
    LANG_OVERRIDE_BACKUP="/root/zorvexpro_lang_override_backup"
...
function remove_bot() {
    ...
    BOT_DIR="/var/www/html/zorvexbotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m Zorvex Bot is not installed (/var/www/html/zorvexbotconfig not found)." | tee -a "$LOG_FILE"
        echo -e "\e[33mNothing to remove. Exiting...\033[0m" | tee -a "$LOG_FILE"
        sleep 2
        exit 1
    fi
    read -p "Are you sure you want to remove Zorvex Bot and its dependencies? (y/n): " choice
    if [[ ! "$choice" =~ ^[Yy]$ ]]; then
        echo "Aborting..." | tee -a "$LOG_FILE"
        exit 0
    fi
    echo "Removing Zorvex Bot..." | tee -a "$LOG_FILE"
    if command -v crontab >/dev/null 2>&1 || [ -x /usr/bin/crontab ]; then
        ...
        echo -e "\e[92mRemoved Zorvex cron jobs.\033[0m" | tee -a "$LOG_FILE"
    fi
    CONFIG_PATH="/var/www/html/zorvexbotconfig/config.php"
    if [ -f "$CONFIG_PATH" ]; then
    ...
    # Clear Zorvex install state so a fresh install is allowed afterwards
    sudo rm -rf /root/confzorvex
    echo -e "\e[92mZorvex Bot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}
...
function migrate_to_pro() {
    ...
    OLD_BOT_DIR="/var/www/html/zorvexconfig"
    ...
    BACKUP_FILE="/root/zorvex_backup.sql"
    if [ ! -f "$BACKUP_FILE" ]; then
        echo -e "\033[31m[ERROR] Backup file not found at $BACKUP_FILE\033[0m"
        echo -e "\033[33mPlease run the 'zorvex' command (Free Version Script) and use option 4 to create a backup.\033[0m"
        exit 1
    else
    ...
    ROOT_CRED_FILE="/root/confzorvex/dbrootzorvex.txt"
    ROOT_PASS=""
    ROOT_USER="root"
    ...
    OLD_CONFIG="/var/www/html/zorvexconfig/config.php"
    ...
    NEW_BOT_DIR="/var/www/html/zorvexbotconfig"
    rm -rf "$OLD_BOT_DIR"
    mkdir -p "$NEW_BOT_DIR"
    ZIP_URL="https://github.com/T3chHash/zorvex/archive/refs/heads/main.zip"
    TEMP_DIR="/tmp/zorvex_mig"
    mkdir -p "$TEMP_DIR"
    run_step "Downloading Zorvex source" "wget -q -O '$TEMP_DIR/bot.zip' '$ZIP_URL'" \
        || { show_step_error; echo -e "\033[31mError: Failed to download Zorvex source.\033[0m"; exit 1; }
...
EOF
    ...
    echo -e "\033[33mUse command 'zorvex' to manage the bot from now on.\033[0m"
    echo ""
}
...
print_usage() {
  ...
  Zorvex - management script

  Usage:
    zorvex [command] [options]

  Commands:
    install            Install Zorvex
    update             Update Zorvex
    remove             Remove Zorvex
